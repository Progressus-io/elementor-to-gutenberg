...
class Batch_Convert_Wizard {
	...
	private function maybe_switch_theme( bool $change_theme, string $new_theme, bool $copy_custom_css, string $mode ) {
		...
		if ( ! isset( $themes[ $new_theme ] ) ) {
			...
			$themes = wp_get_themes();
			if ( ! isset( $themes[ $new_theme ] ) ) {
				return new WP_Error( 'metg-theme-missing', esc_html__( 'The selected theme is not available.', 'layoutbridge-block-migration' ) );
			}
		}
		...
		if ( get_stylesheet() !== $new_theme ) {
			return new WP_Error( 'metg-theme-switch-failed', esc_html__( 'Unable to switch to the selected theme.', 'layoutbridge-block-migration' ) );
		}
