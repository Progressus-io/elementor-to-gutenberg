...
class Elementor_Fonts_Service {
	 ...
	 */
	private const OPTION_NAME = 'blockshift_font_requirements';
	...
	public static function get_post_fonts_url( int $post_id ): string {
		...
		$fonts = get_post_meta( $post_id, '_blockshift_used_fonts', true );
		if ( ! is_array( $fonts ) || empty( $fonts ) ) {
			return '';
		}

		$stored_hash = (string) get_post_meta( $post_id, '_blockshift_used_fonts_hash', true );
		if ( '' === $stored_hash ) {
			$stored_hash = md5( (string) wp_json_encode( $fonts ) );
		}

		$cache_key = 'blockshift_fonts_url_' . $post_id . '_' . md5( $stored_hash . '|' . self::get_alias_map_version() );
	...
	private static function get_font_alias_map(): array {
		$raw = get_option( 'blockshift_font_alias_map', '' );
	...
	private static function get_alias_map_version(): string {
		$version = get_option( 'blockshift_font_alias_map_version', '' );
		if ( is_string( $version ) && '' !== trim( $version ) ) {
			return trim( $version );
		}

		$map = get_option( 'blockshift_font_alias_map', '' );
