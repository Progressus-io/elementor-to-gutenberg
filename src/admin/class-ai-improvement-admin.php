...
class AI_Improvement_Admin {
	...
	public function enqueue_assets( string $_hook ): void { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
		...
		wp_localize_script(
			...
			'metgAiImprove',
			array(
				'processingLabel'     => __( 'Processing…', 'migrate-off-elementor' ),
				'improvingLabel'      => __( 'Improving with AI…', 'migrate-off-elementor' ),
				'refiningLabel'       => __( 'Refining with AI…', 'migrate-off-elementor' ),
				'mobileLabel'         => __( 'Improving mobile with AI…', 'migrate-off-elementor' ),
				'ajaxUrl'             => admin_url( 'admin-ajax.php' ),
				'feedbackNonce'       => wp_create_nonce( AI_Enhancement_Admin::FEEDBACK_NONCE ),
				'targetId'            => $target_id_asset,
				'sourceId'            => $source_id_asset,
				'feedbackTitle'       => __( 'How did AI Enhancement go?', 'migrate-off-elementor' ),
				'feedbackIssue'       => __( 'Issue type', 'migrate-off-elementor' ),
				'feedbackIssueDetail' => __( 'Describe the issue', 'migrate-off-elementor' ),
				'feedbackNote'        => __( 'Additional notes', 'migrate-off-elementor' ),
				'feedbackConsent'     => __( 'I consent to sending this anonymised AI enhancement report to the plugin developer for quality improvement. No passwords, API keys, or user data are included.', 'migrate-off-elementor' ),
				...
				'feedbackSuccess'     => __( 'Thank you! Feedback submitted.', 'migrate-off-elementor' ),
				'feedbackNoIssue'     => __( 'No issue', 'migrate-off-elementor' ),
				'feedbackLayout'      => __( 'Layout issues after AI', 'migrate-off-elementor' ),
				'feedbackMissing'     => __( 'Wrong or missing content', 'migrate-off-elementor' ),
				'feedbackCss'         => __( 'CSS / styling problems', 'migrate-off-elementor' ),
				'feedbackQuality'     => __( 'AI output quality', 'migrate-off-elementor' ),
				'feedbackOther'       => __( 'Other', 'migrate-off-elementor' ),
			)
		);
	}
	...
	public function register_menu(): void {
		...
		add_submenu_page(
			null,
			esc_html__( 'Improve Converted Page with AI', 'migrate-off-elementor' ),
			esc_html__( 'Improve Converted Page with AI', 'migrate-off-elementor' ),
			'edit_pages',
			self::MENU_SLUG,
	...
	public function render_page(): void {
		...
		if ( $target_id <= 0 ) {
			wp_die( esc_html__( 'Missing converted Gutenberg page ID.', 'migrate-off-elementor' ) );
		}

		$target_post = get_post( $target_id );
		if ( ! $target_post instanceof WP_Post ) {
			wp_die( esc_html__( 'Converted Gutenberg page not found.', 'migrate-off-elementor' ) );
		}

		if ( ! current_user_can( 'edit_post', $target_id ) ) {
			wp_die( esc_html__( 'You do not have permission to edit this page.', 'migrate-off-elementor' ) );
		}
		...
		if ( $source_id <= 0 ) {
			wp_die( esc_html__( 'Source Elementor page ID could not be resolved.', 'migrate-off-elementor' ) );
		}

		$stored_source_id = (int) get_post_meta( $target_id, '_metg_source_id', true );
		if ( $stored_source_id > 0 && $stored_source_id !== $source_id ) {
			wp_die( esc_html__( 'The selected source and target page mapping is invalid.', 'migrate-off-elementor' ) );
		}

		$source_post = get_post( $source_id );
		if ( ! $source_post instanceof WP_Post ) {
			wp_die( esc_html__( 'Source Elementor page not found.', 'migrate-off-elementor' ) );
		}
	...
	public function handle_auto_improve(): void {
		if ( ! current_user_can( 'edit_pages' ) ) {
			wp_die( esc_html__( 'You do not have permission to perform this action.', 'migrate-off-elementor' ) );
		}
		...
		if ( $target_id <= 0 || $source_id <= 0 ) {
			wp_die( esc_html__( 'Source or target page is missing.', 'migrate-off-elementor' ) );
		}
	...
	public static function run_improvement( int $source_id, int $target_id ): array {
		...
		if ( ! $screenshot_result['success'] ) {
			return $failure(
				sprintf(
					...
					__( 'Screenshots could not be generated, so AI enhancement was not run: %s', 'migrate-off-elementor' ),
		...
		if ( '' === trim( $gutenberg_result ) ) {
			...
			return $failure(
				__( 'No valid Gutenberg content could be parsed from the AI response.', 'migrate-off-elementor' ),
				'ai_parse_failed'
			);
		}
	...
	private static function check_page_accessibility( int $source_id, int $target_id ): array {
		...
		if ( file_exists( (string) ABSPATH . '.maintenance' ) || ( defined( 'WP_MAINTENANCE_MODE' ) && WP_MAINTENANCE_MODE ) ) {
			return $deny( __( 'The website is in maintenance mode, so the screenshot service cannot load it. Disable maintenance mode and try again.', 'migrate-off-elementor' ) );
		}
		...
		if ( $is_library ) {
			...
			if ( 'publish' !== (string) get_post_status( $target_id ) ) {
				return $deny( __( 'The converted page is not published yet. Publish it so the screenshot service can load it, then try again.', 'migrate-off-elementor' ) );
			}
			if ( '' !== (string) get_post_field( 'post_password', $target_id ) ) {
				return $deny( __( 'The converted page is password-protected. Remove the password so the screenshot service can load it, then try again.', 'migrate-off-elementor' ) );
			}
			$page_url = (string) get_permalink( $target_id );
		}

		if ( '' === $page_url ) {
			return $deny( __( 'The public URL of the converted page could not be resolved.', 'migrate-off-elementor' ) );
		}
		...
		if ( self::is_non_public_host( $host ) ) {
			return $deny(
				sprintf(
					...
					__( 'This site (%s) is only reachable on your local network, so the remote screenshot service cannot open it. AI Enhancement needs a publicly accessible URL — run it on a live or staging site.', 'migrate-off-elementor' ),
					$host
				)
			);
		}
			...
			if ( $code >= 400 ) {
				return $deny(
					sprintf(
						...
						__( 'The converted page (%1$s) returned HTTP status %2$d, so the screenshot service cannot load it. Check that the page is public and not behind login or HTTP authentication.', 'migrate-off-elementor' ),
						$page_url,
						$code
					)
				);
	...
	public static function run_refinement( int $source_id, int $target_id, string $focus_instruction ): array {
		...
		if ( ! $screenshot_result['success'] ) {
			return $failure(
				sprintf(
					/* translators: %s: screenshot error details */
					__( 'Screenshots could not be generated, so AI refinement was not run: %s', 'migrate-off-elementor' ),
					$screenshot_result['error']
				),
				'screenshot_failed'
			);
		...
		if ( '' === trim( $gutenberg_result ) ) {
			...
			return $failure(
				__( 'No valid Gutenberg content could be parsed from the AI refinement response.', 'migrate-off-elementor' ),
				'ai_parse_failed'
			);
		}
	...
	public static function run_mobile_improvement( int $source_id, int $target_id ): array {
		...
		if ( ! $screenshot_result['success'] ) {
			return $failure(
				sprintf(
					/* translators: %s: screenshot error details */
					__( 'Screenshots could not be generated, so mobile AI enhancement was not run: %s', 'migrate-off-elementor' ),
					$screenshot_result['error']
				)
			);
		}
	...
	private function render_notice( string $notice_code, int $target_id = 0 ): void {
		...
		$detail_prefixes = array(
			'ai_failed'         => esc_html__( 'Claude API call failed', 'migrate-off-elementor' ),
			'mobile_failed'     => esc_html__( 'Mobile improvement failed', 'migrate-off-elementor' ),
			'page_inaccessible' => esc_html__( 'AI enhancement was not run because this page is not accessible', 'migrate-off-elementor' ),
			'screenshot_failed' => esc_html__( 'AI enhancement was not run because screenshots could not be generated', 'migrate-off-elementor' ),
		);
		...
		if ( isset( $detail_prefixes[ $notice_code ] ) ) {
			...
			$msg    = '' !== $ai_error
				...
				? sprintf( esc_html__( '%1$s: %2$s', 'migrate-off-elementor' ), $prefix, esc_html( $ai_error ) )
				: $prefix . '.';
			?>
		...
		$messages = array(
			'updated'                 => array( 'success', esc_html__( 'Page updated and AI CSS appended successfully.', 'migrate-off-elementor' ) ),
			'missing_gutenberg'       => array( 'error', esc_html__( 'Gutenberg result is required before updating.', 'migrate-off-elementor' ) ),
			'css_append_failed'       => array( 'error', esc_html__( 'Could not append CSS because the external CSS file for this page could not be resolved.', 'migrate-off-elementor' ) ),
			'update_failed'           => array( 'error', esc_html__( 'Failed to update Gutenberg page content.', 'migrate-off-elementor' ) ),
			'invalid_mapping'         => array( 'error', esc_html__( 'Source and target mapping validation failed.', 'migrate-off-elementor' ) ),
			'screenshots_regenerated' => array( 'success', esc_html__( 'Screenshots regenerated successfully.', 'migrate-off-elementor' ) ),
			'screenshots_failed'      => array( 'error', esc_html__( 'Screenshot regeneration failed. Check the screenshot service settings and connectivity.', 'migrate-off-elementor' ) ),
			'ai_parse_failed'         => array( 'error', esc_html__( 'Claude returned a response but no valid Gutenberg content could be parsed.', 'migrate-off-elementor' ) ),
			'refined'                 => array( 'success', esc_html__( 'Page refined successfully. Fresh screenshots were captured before this run.', 'migrate-off-elementor' ) ),
			'mobile_improved'         => array( 'success', esc_html__( 'Mobile CSS improved successfully. Desktop styles were not modified.', 'migrate-off-elementor' ) ),
			'mobile_failed'           => array( 'error', esc_html__( 'Mobile improvement failed. Check the screenshot service and Claude API settings.', 'migrate-off-elementor' ) ),
		);
	...
	private function render_form( WP_Post $target_post, WP_Post $source_post, array $workspace ): void {
		...
		$pill_map = array(
			AI_Remediation_Screenshot_Meta_Service::STATUS_SUCCESS       => array( 'success', esc_html__( 'Generated', 'migrate-off-elementor' ) ),
			AI_Remediation_Screenshot_Meta_Service::STATUS_FAILED        => array( 'error', esc_html__( 'Failed', 'migrate-off-elementor' ) ),
			AI_Remediation_Screenshot_Meta_Service::STATUS_PENDING       => array( 'pending', esc_html__( 'Pending', 'migrate-off-elementor' ) ),
			AI_Remediation_Screenshot_Meta_Service::STATUS_NOT_GENERATED => array( 'neutral', esc_html__( 'Not generated', 'migrate-off-elementor' ) ),
		);
		...
		$suggestions = array(
			__( 'Fix hero section spacing and alignment', 'migrate-off-elementor' ),
			__( 'Match typography — font sizes and weights', 'migrate-off-elementor' ),
			__( 'Improve button styles and colors', 'migrate-off-elementor' ),
			__( 'Fix colors and contrast', 'migrate-off-elementor' ),
			__( 'Fix image sizing and alignment', 'migrate-off-elementor' ),
			__( 'Fix section padding and spacing', 'migrate-off-elementor' ),
			__( 'Improve heading styles', 'migrate-off-elementor' ),
			__( 'Fix navigation menu styling', 'migrate-off-elementor' ),
		);
		...
		<div class="metg-ai-page">
			...
			<div class="metg-ai-header">
				<div class="metg-ai-header-nav">
					<a href="<?php echo esc_url( $enhancement_url ); ?>" class="metg-ai-back-link">&#8592; <?php esc_html_e( 'Back to AI Enhancement', 'migrate-off-elementor' ); ?></a>
				</div>
				<div class="metg-ai-header-main">
					...
					<div class="metg-ai-header-actions">
						<?php if ( $source_prev_url ) : ?>
							<a href="<?php echo esc_url( $source_prev_url ); ?>" target="_blank" rel="noopener" class="button"><?php esc_html_e( 'View Source &#8599;', 'migrate-off-elementor' ); ?></a>
						<?php endif; ?>
						<?php if ( $target_prev_url ) : ?>
							<a href="<?php echo esc_url( $target_prev_url ); ?>" target="_blank" rel="noopener" class="button"><?php esc_html_e( 'Preview &#8599;', 'migrate-off-elementor' ); ?></a>
						<?php endif; ?>
						<?php if ( '' !== $last_improved ) : ?>
							<button type="button" id="metg-ai-feedback-btn" class="button"><?php esc_html_e( 'Send Feedback', 'migrate-off-elementor' ); ?></button>
						<?php endif; ?>
						<a href="<?php echo esc_url( $target_edit_url ); ?>" class="button button-primary"><?php esc_html_e( 'Edit in Gutenberg', 'migrate-off-elementor' ); ?></a>
					</div>
				</div>
			</div>
			...
			<div class="metg-ai-layout">
				...
				<div class="metg-ai-main">
					...
					<div class="metg-ai-card">
						<div class="metg-ai-card-header">
							<h2><?php esc_html_e( 'Screenshots', 'migrate-off-elementor' ); ?></h2>
							...
							<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="metg-inline-form">
								...
								<button type="submit" class="button button-small"><?php esc_html_e( 'Regenerate', 'migrate-off-elementor' ); ?></button>
							</form>
						</div>

						<div class="metg-ai-tabs" role="tablist">
							<button type="button" class="metg-ai-tab metg-ai-tab--active" role="tab" data-tab="desktop" aria-selected="true"><?php esc_html_e( 'Desktop', 'migrate-off-elementor' ); ?></button>
							<button type="button" class="metg-ai-tab" role="tab" data-tab="mobile" aria-selected="false"><?php esc_html_e( 'Mobile', 'migrate-off-elementor' ); ?></button>
						</div>

						<div class="metg-ai-tab-panel" data-panel="desktop">
							<div class="metg-ai-compare-grid">
								<div class="metg-compare-side">
									<div class="metg-compare-label"><?php esc_html_e( 'Elementor (Original)', 'migrate-off-elementor' ); ?></div>
									<?php
									...
									if ( $d_ele_first ) :
										...
										<div class="metg-screenshot-thumb-wrap" data-urls="<?php echo esc_attr( wp_json_encode( $d_ele_urls ) ); ?>">
											...
											<button type="button" class="metg-screenshot-zoom-btn" aria-label="<?php esc_attr_e( 'View full screenshot', 'migrate-off-elementor' ); ?>">&#x2922;</button>
										</div>
									<?php else : ?>
										<div class="metg-screenshot-empty"><?php esc_html_e( 'No desktop screenshot yet', 'migrate-off-elementor' ); ?></div>
									<?php endif; ?>
								</div>
								<div class="metg-compare-side">
									<div class="metg-compare-label"><?php esc_html_e( 'Gutenberg (Converted)', 'migrate-off-elementor' ); ?></div>
									<?php
						...
						<div class="metg-ai-tab-panel" data-panel="mobile" hidden>
							<div class="metg-ai-compare-grid">
								<div class="metg-compare-side">
									<div class="metg-compare-label"><?php esc_html_e( 'Elementor Mobile', 'migrate-off-elementor' ); ?></div>
									<?php
									...
									<?php else : ?>
										<div class="metg-screenshot-empty"><?php esc_html_e( 'No mobile screenshot yet', 'migrate-off-elementor' ); ?></div>
									<?php endif; ?>
								</div>
								<div class="metg-compare-side">
									<div class="metg-compare-label"><?php esc_html_e( 'Gutenberg Mobile', 'migrate-off-elementor' ); ?></div>
									<?php
						...
						<div class="metg-ai-card-footer">
							<?php if ( '' !== $screenshot_generated_at ) : ?>
								...
								printf( esc_html__( 'Last captured: %s', 'migrate-off-elementor' ), esc_html( $screenshot_generated_at ) );
								?>
							<?php endif; ?>
							<?php if ( ! $service_configured ) : ?>
								<span class="metg-warning-inline"><?php esc_html_e( 'Screenshot service not configured — see Settings.', 'migrate-off-elementor' ); ?></span>
							<?php endif; ?>
						</div>
						<?php endif; ?>
					</div>
					...
					<div class="metg-ai-card">
						<?php if ( '' === $last_improved ) : ?>
							<div class="metg-ai-card-header">
								<h2><?php esc_html_e( 'AI Improvement', 'migrate-off-elementor' ); ?></h2>
								<span class="metg-status-pill metg-status-pill--neutral"><?php esc_html_e( 'Not yet run', 'migrate-off-elementor' ); ?></span>
							</div>
							<div class="metg-ai-card-body">
								<p class="metg-card-desc"><?php esc_html_e( 'Analyse and improve the converted page using AI. The page content and CSS will be updated automatically.', 'migrate-off-elementor' ); ?></p>
								<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" id="metg-ai-improve-form">
									...
									<input type="hidden" name="target_id" value="<?php echo esc_attr( (string) $target_id ); ?>" />
									<input type="hidden" name="source_id" value="<?php echo esc_attr( (string) $source_id ); ?>" />
									<?php submit_button( esc_html__( 'Improve with AI', 'migrate-off-elementor' ), 'primary', 'metg_auto_improve_submit', false ); ?>
								</form>
							</div>
						<?php else : ?>
							<div class="metg-ai-card-header">
								<h2><?php esc_html_e( 'Refine with AI', 'migrate-off-elementor' ); ?></h2>
								<span class="metg-status-pill metg-status-pill--success"><?php esc_html_e( 'Improved', 'migrate-off-elementor' ); ?></span>
							</div>
							<div class="metg-ai-card-body">
								<p class="metg-card-desc"><?php esc_html_e( 'Tell AI exactly what to focus on. Fresh screenshots are captured automatically before each run.', 'migrate-off-elementor' ); ?></p>
								<div class="metg-refine-suggestions">
									<span class="metg-refine-suggestions-label"><?php esc_html_e( 'Quick suggestions:', 'migrate-off-elementor' ); ?></span>
								...
								<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" id="metg-ai-refine-form">
									...
									<textarea
										...
										rows="4"
										placeholder="<?php echo esc_attr__( 'Describe what needs fixing, e.g. "The hero section spacing is too tight and the heading font is too small"', 'migrate-off-elementor' ); ?>"
										class="metg-refine-textarea"
									></textarea>
									<?php submit_button( esc_html__( 'Refine with AI', 'migrate-off-elementor' ), 'primary', 'metg_refine_submit', false ); ?>
								</form>
							</div>
						<?php endif; ?>
					</div>
					...
					<div class="metg-ai-card">
						<div class="metg-ai-card-header">
							<h2><?php esc_html_e( 'Mobile Optimisation', 'migrate-off-elementor' ); ?></h2>
						...
						<div class="metg-ai-card-body">
							<p class="metg-card-desc"><?php esc_html_e( 'Compares mobile screenshots and generates @media query CSS. Desktop styles and block content are not modified.', 'migrate-off-elementor' ); ?></p>
							<?php if ( ! $has_mobile_shots ) : ?>
								<div class="metg-notice metg-notice--warning">
									<?php esc_html_e( 'Mobile screenshots are missing — click Regenerate in the Screenshots card above before running this pass.', 'migrate-off-elementor' ); ?>
								</div>
							<?php endif; ?>
							<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" id="metg-ai-mobile-improve-form">
								...
								<input type="hidden" name="target_id" value="<?php echo esc_attr( (string) $target_id ); ?>" />
								<input type="hidden" name="source_id" value="<?php echo esc_attr( (string) $source_id ); ?>" />
								<?php submit_button( esc_html__( 'Improve Mobile with AI', 'migrate-off-elementor' ), 'secondary', 'metg_mobile_improve_submit', false ); ?>
							</form>
						</div>
					</div>
				...
				<aside class="metg-ai-sidebar">
					...
					<div class="metg-ai-card">
						<div class="metg-ai-card-header">
							<h2><?php esc_html_e( 'Page Details', 'migrate-off-elementor' ); ?></h2>
						</div>
						<div class="metg-ai-card-body">
							<dl class="metg-ai-dl">
								<dt><?php esc_html_e( 'Source (Elementor)', 'migrate-off-elementor' ); ?></dt>
								<dd>
									<a href="<?php echo esc_url( $source_edit_url ); ?>"><?php echo esc_html( $source_title ); ?></a>
									<?php if ( $source_prev_url ) : ?>
										<a href="<?php echo esc_url( $source_prev_url ); ?>" target="_blank" rel="noopener" class="metg-ext-link" title="<?php esc_attr_e( 'Preview', 'migrate-off-elementor' ); ?>">&#8599;</a>
									<?php endif; ?>
								</dd>
								<dt><?php esc_html_e( 'Target (Gutenberg)', 'migrate-off-elementor' ); ?></dt>
								<dd>
									<a href="<?php echo esc_url( $target_edit_url ); ?>"><?php echo esc_html( $target_title ); ?></a>
									<?php if ( $target_prev_url ) : ?>
										<a href="<?php echo esc_url( $target_prev_url ); ?>" target="_blank" rel="noopener" class="metg-ext-link" title="<?php esc_attr_e( 'Preview', 'migrate-off-elementor' ); ?>">&#8599;</a>
									<?php endif; ?>
								</dd>
								<dt><?php esc_html_e( 'Source ID', 'migrate-off-elementor' ); ?></dt>
								<dd><?php echo esc_html( (string) $source_id ); ?></dd>
								<dt><?php esc_html_e( 'Target ID', 'migrate-off-elementor' ); ?></dt>
					...
					<div class="metg-ai-card">
						<div class="metg-ai-card-header">
							<h2><?php esc_html_e( 'AI Status', 'migrate-off-elementor' ); ?></h2>
						</div>
						<div class="metg-ai-card-body">
							<dl class="metg-ai-dl">
								<dt><?php esc_html_e( 'Desktop', 'migrate-off-elementor' ); ?></dt>
								<dd>
									<?php if ( '' !== $last_improved ) : ?>
										<?php echo esc_html( $last_improved ); ?>
									<?php else : ?>
										<span class="metg-muted"><?php esc_html_e( 'Not yet run', 'migrate-off-elementor' ); ?></span>
									<?php endif; ?>
								</dd>
								<dt><?php esc_html_e( 'Mobile', 'migrate-off-elementor' ); ?></dt>
								<dd>
									<?php if ( '' !== $last_mobile_improved ) : ?>
			...
			<div id="metg-lightbox" class="metg-lightbox" hidden role="dialog" aria-modal="true" aria-label="<?php esc_attr_e( 'Screenshot viewer', 'migrate-off-elementor' ); ?>">
				<div class="metg-lightbox-overlay" id="metg-lightbox-overlay"></div>
				<div class="metg-lightbox-panel">
					<div class="metg-lightbox-toolbar">
						<a id="metg-lightbox-open" href="#" target="_blank" rel="noopener" class="metg-lightbox-open-link"><?php esc_html_e( 'Open full image &#8599;', 'migrate-off-elementor' ); ?></a>
						<button type="button" id="metg-lightbox-close" class="metg-lightbox-close" aria-label="<?php esc_attr_e( 'Close', 'migrate-off-elementor' ); ?>">&#x2715;</button>
					</div>
			...
			<div id="metg-ai-loader" hidden>
				<div class="metg-ai-loader-card">
					...
					<div>
						<strong class="metg-ai-loader-title"><?php esc_html_e( 'Improving with AI&#8230;', 'migrate-off-elementor' ); ?></strong>
						<span class="metg-ai-loader-message"><?php esc_html_e( 'Analysing page structure and generating improvements. This may take up to 2 minutes.', 'migrate-off-elementor' ); ?></span>
					</div>
				</div>
			</div>
