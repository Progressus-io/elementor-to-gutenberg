(function (window, document) {
  ...
  class WizardApp {
    ...
    resetSelectionForMode(mode) {
      ...
      if (mode === "auto") {
        ...
        this.state.selectedHeaderIds = new Set(
          this.getTemplatesFor("header").map((template) => Number(template.id)),
        );
        this.state.selectedFooterIds = new Set(
          this.getTemplatesFor("footer").map((template) => Number(template.id)),
        );
    ...
    isCurrentThemeBlock() {
      return !!(
        this.themes.currentTheme && this.themes.currentTheme.isBlockTheme
      );
    }
      ...
      switch (step) {
        ...
        case "select": {
          const summary = formatString(
            this.strings.selectionSummary || "%1$d selected / %2$d total",
        ...
        case "templates":
          return (
            this.strings.headerFooterStepTitle || "Header & Footer Templates"
          );
        case "conflicts":
    ...
    renderModeStep() {
      ...
      modes.forEach((mode) => {
        const card = createElement(
          "label",
          "ele2gb-mode-card" +
            (this.state.modeSelection === mode.key ? " is-active" : ""),
        );
        ...
        const subtextKey =
          mode.key === "auto" ? "modeAutoSubtext" : "modeCustomSubtext";
        ...
        if (preflight.eligibleCount !== undefined) {
          ...
          card.appendChild(
            createElement("small", "ele2gb-mode-context-line", contextLine),
          );
        }
    ...
    renderThemeStep() {
      ...
      const currentTheme = {
        ...
        isInstalled: true,
        screenshot:
          this.themes.currentTheme && this.themes.currentTheme.screenshot
      ...
      const selectedName =
        (selectedTheme && selectedTheme.name) || this.getCurrentThemeName();
      const selectedSummary = this.willChangeTheme()
        ...
        : formatString(
            this.strings.themeUsingCurrentSummary || "Using current theme: %s",
            this.getCurrentThemeName(),
          );
      container.appendChild(
        createElement("p", "ele2gb-theme-selected-summary", selectedSummary),
      );
    ...
    renderSelectStep() {
      ...
      if (this.postTypes.length > 1) {
        ...
        masterCheckbox.checked =
          allPageIds.length > 0 &&
          allPageIds.every((id) => this.state.selectedPageIds.has(id));
        ...
        masterLabel.appendChild(
          createElement(
            "span",
            null,
            this.strings.selectAllAcrossTypes || "Select all across all types",
          ),
        );
        ...
        this.postTypes.forEach((pt) => {
          ...
          tab.className =
            "ele2gb-tab" +
            (this.state.activeTab === pt.slug ? " is-active" : "");
      ...
      visiblePages.forEach((page) => {
        ...
        const badgeInfo =
          STATUS_BADGES[page.conversionStatus] || STATUS_BADGES.not_converted;
        const badge = createElement(
          "span",
          "ele2gb-status-badge " + (badgeInfo ? badgeInfo.className : ""),
        );
        ...
        const hasAnyWarning =
          unsupTypes.length > 0 || warnings.hasDynamic || warnings.hasAnimation;
        if (hasAnyWarning) {
          ...
          trigger.setAttribute(
            "aria-label",
            this.strings.compatShowDetails || "Show compatibility details",
          );
    ...
    renderTemplatesGroup(type, label) {
      ...
      if (!templates.length) {
        const noneMessage =
          type === "header"
            ? this.strings.noHeadersFound || "No header templates detected."
            : this.strings.noFootersFound || "No footer templates detected.";
        container.appendChild(
      ...
      templates.forEach((template) => {
        ...
        if (metaParts.length) {
          titleTd.appendChild(
            createElement("div", "ele2gb-template-meta", metaParts.join(" · ")),
          );
        }
      ...
      const labelText =
        type === "header"
          ? this.strings.defaultHeaderLabel || "Default header after conversion"
    ...
    renderPagination() {
      ...
      pagination.appendChild(
        createElement("span", null, this.state.tablePage + " / " + totalPages),
      );
    ...
    buildReviewStatTile(value, label) {
      ...
      tile.appendChild(
        createElement("div", "ele2gb-review-stat-value", String(value)),
      );
          ...
          (body) => {
            ...
            if (this.willChangeTheme()) {
              const themeName =
                ...
                "";
              ul.appendChild(
                createElement("li", null, "Switching to: " + themeName),
              );
              ...
              ul.appendChild(
                createElement(
                  "li",
                  null,
                  (this.strings.themeKeepCurrent || "Keep current theme") +
                    ": " +
                    this.getCurrentThemeName(),
                ),
              );
          ...
          this.buildReviewSection(
            ...
            (body) => {
              ...
              const defaultHeader = headerCount
                ? this.getTemplateById(this.state.defaultHeaderId)
                : null;
              const defaultFooter = footerCount
                ? this.getTemplateById(this.state.defaultFooterId)
                : null;
    ...
    renderProgressStep() {
      ...
      if (!this.state.job) {
        container.appendChild(
          createElement("p", null, this.strings.processing || "Processing…"),
        );
        return container;
      }
      ...
      if (this.state.resumed) {
        container.appendChild(
          createElement(
            ...
            this.strings.resumeJob || "Resuming an active conversion job.",
          ),
        );
      }
      ...
      const makeTile = function (value, label, modifier) {
        ...
        tile.appendChild(
          createElement("div", "ele2gb-stat-tile-value", String(value)),
        );
      ...
      summary.appendChild(
        makeTile(skippedCount, this.strings.skipped || "Skipped", "muted"),
      );
      summary.appendChild(
        makeTile(
          errorCount,
    ...
    buildActionPill(opts) {
      ...
      el.className =
        "ele2gb-action-pill ele2gb-action-pill--" + (opts.variant || "default");
      if (opts.href) {
        el.href = opts.href;
        if (opts.external) {
          el.target = "_blank";
      ...
      if (opts.label) {
        el.appendChild(
          createElement("span", "ele2gb-action-pill-label", opts.label),
        );
      }
      return el;
    }
    ...
    buildResultsTable(results) {
      ...
      results.forEach((result) => {
        ...
        if (metaParts.length) {
          titleTd.appendChild(
            createElement("div", "ele2gb-result-meta", metaParts.join(" · ")),
          );
        }
        tr.appendChild(titleTd);
        ...
        const badgeInfo =
          STATUS_BADGES[resultConfig.badge] || STATUS_BADGES.not_converted;
        const badge = createElement(
          "span",
        ...
        if (result.status === "error" && result.type === "page") {
          actionGroup.appendChild(
            this.buildActionPill({
              ...
              iconPath: ["M1 4v6h6", "M3.51 15a9 9 0 102.13-9.36L1 10"],
              onClick: (event) => {
                event.preventDefault();
      ...
      if (successes.length) {
        container.appendChild(
          createElement(
            ...
            this.strings.resultsCompleted || "Completed successfully",
          ),
        );
    ...
    renderAiImproveStep() {
      ...
      if (!ai || !ai.started) {
        ...
        const makeRow = function (ok, label) {
          ...
          row.appendChild(
            createElement(
              "span",
              "ele2gb-ai-readiness-status" + (ok ? "" : " is-invalid"),
              label,
            ),
          );
          return row;
      ...
      ai.pages.forEach((page, i) => {
        ...
        if (page.status === "pending") {
          ...
          statusTd.appendChild(
            this.makeAiStatusBadge("done", this.strings.aiStatusDone || "Done"),
          );
          ...
          if (page.error) {
            statusTd.appendChild(
              createElement("p", "ele2gb-ai-error-msg", page.error),
            );
          }
    ...
    buildWarningPopover(page) {
      ...
      if (unsupTypes.length > 0) {
        if (!first)
          body.appendChild(createElement("hr", "ele2gb-warn-popover-divider"));
        first = false;
        ...
        sTitle.textContent =
          "⚠ " + (this.strings.warnTitleUnsupported || "Unsupported Widgets");
