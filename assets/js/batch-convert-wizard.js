( function ( window, document ) {
	...
	class WizardApp {
		constructor( rootEl, config ) {
			...
			this.pages = Array.isArray( config.pages )
				? config.pages.slice()
				: [];
			this.postTypes = Array.isArray( config.postTypes )
				? config.postTypes.slice()
				: [];
			this.themes = config.themes || {
				currentTheme: null,
				installedBlockThemes: [],
				suggestedCoreThemes: [],
			};
			...
			this.state = {
				...
				searchQuery: '',
				activeTab: this.postTypes.length
					? this.postTypes[ 0 ].slug
					: '',
			};
		...
		resetSelectionForMode( mode ) {
			const defaultHeader =
				this.templates && this.templates.defaults
					? Number( this.templates.defaults.header ) || 0
					: 0;
			const defaultFooter =
				this.templates && this.templates.defaults
					? Number( this.templates.defaults.footer ) || 0
					: 0;
			...
			if ( mode === 'auto' ) {
				...
				this.state.selectedHeaderIds = new Set(
					this.getTemplatesFor( 'header' ).map( ( template ) =>
						Number( template.id )
					)
				);
				this.state.selectedFooterIds = new Set(
					this.getTemplatesFor( 'footer' ).map( ( template ) =>
						Number( template.id )
					)
				);
				this.state.defaultHeaderId = this.pickDefaultTemplate(
					'header',
					this.state.selectedHeaderIds,
					defaultHeader
				);
				this.state.defaultFooterId = this.pickDefaultTemplate(
					'footer',
					this.state.selectedFooterIds,
					defaultFooter
				);
		...
		getTemplatesFor( type ) {
			if ( type === 'header' ) {
				return Array.isArray( this.templates.headers )
					? this.templates.headers
					: [];
			}
			if ( type === 'footer' ) {
				return Array.isArray( this.templates.footers )
					? this.templates.footers
					: [];
			}
			return [];
		}
		...
		getTemplateById( id ) {
			...
			const header = this.getTemplatesFor( 'header' ).find(
				( template ) => Number( template.id ) === targetId
			);
			if ( header ) {
				return header;
			}
			return (
				this.getTemplatesFor( 'footer' ).find(
					( template ) => Number( template.id ) === targetId
				) || null
			);
		}
		...
		getCurrentThemeSlug() {
			return (
				( this.themes.currentTheme && this.themes.currentTheme.slug ) ||
				''
			);
		}
		...
		getCurrentThemeName() {
			return (
				( this.themes.currentTheme && this.themes.currentTheme.name ) ||
				''
			);
		}
		...
		isCurrentThemeBlock() {
			return !! (
				...
				this.themes.currentTheme.isBlockTheme
			);
		}
		...
		willChangeTheme() {
			const targetSlug =
				this.state.selectedThemeSlug || this.getCurrentThemeSlug();
			return !! (
				targetSlug && targetSlug !== this.getCurrentThemeSlug()
			);
		}
		...
		getSelectedTheme() {
			const slug =
				this.state.selectedThemeSlug || this.getCurrentThemeSlug();
			const installed = Array.isArray( this.themes.installedBlockThemes )
				? this.themes.installedBlockThemes
				: [];
			const suggested = Array.isArray( this.themes.suggestedCoreThemes )
				? this.themes.suggestedCoreThemes
				: [];

			return (
				installed.find( ( theme ) => theme.slug === slug ) ||
				suggested.find( ( theme ) => theme.slug === slug ) ||
				null
			);
		}
		...
		getSelectedTemplates( type ) {
			const set =
				type === 'header'
					? this.state.selectedHeaderIds
					: this.state.selectedFooterIds;
			return this.getTemplatesFor( type ).filter( ( template ) =>
				set.has( template.id )
			);
		}
		...
		hasAnySelection() {
			...
			return (
				...
				this.state.selectedHeaderIds.size > 0 ||
				this.state.selectedFooterIds.size > 0
			);
		}
		...
		getStepTitle( step ) {
			switch ( step ) {
				...
				case 'select': {
					const summary = formatString(
						this.strings.selectionSummary ||
							'%1$d selected / %2$d total',
						this.state.selectedPageIds.size,
						this.pages.length
					);
					return (
		...
		shouldShowSkipConvertedOption() {
			...
			return this.getSelectedPages().some(
				( page ) => page.conversionStatus === 'converted'
			);
		}

		getConflictCount() {
			return this.getSelectedPages().filter(
				( page ) => page.hasConflict
			).length;
		}
		...
		cancelCurrentJob() {
			...
			this.request( 'ele2gb_cancel_job', { jobId: this.state.job.id } )
				.then( ( response ) => {
					...
					this.setNotice(
						'info',
						this.strings.jobCancelled || 'Conversion was cancelled.'
					);
					this.render();
				} )
				.catch( ( error ) => {
					...
					if ( ! json || ! json.success ) {
						const message =
							json && json.data && json.data.message
								? json.data.message
								: this.strings.retryFailed || 'Request failed.';
		...
		startConversion() {
			...
			if (
				...
				! selectedFooters.length
			) {
				this.setNotice(
					'error',
					this.strings.noSelectionError ||
						'Select at least one page or template before continuing.'
				);
				return;
			}
			this.clearNotice();
		...
		retryConversionForPage( pageId ) {
			...
			payload.conflictPolicy = this.state.job
				? this.state.job.conflictPolicy || 'skip'
				: 'skip';
		...
		renderHeader() {
			...
			steps.forEach( ( step, i ) => {
				...
				if ( i < currentIndex ) {
					...
					path.setAttribute(
						'd',
						'M7.5 13.5 4 10l1.4-1.4 2.1 2.1 5.1-5.1L14 7z'
					);
		...
		renderModeStep() {
			...
			const modes = [
				{
					key: 'auto',
					title:
						this.strings.modeAutoTitle ||
						'Convert all pages automatically',
			...
			modes.forEach( ( mode ) => {
				...
				if ( mode.description ) {
					card.appendChild(
						createElement( 'p', null, mode.description )
					);
				}
		...
		getJobWarnings() {
			if (
				! this.state.job ||
				! Array.isArray( this.state.job.warnings )
			) {
				return [];
			}
		...
		renderThemeWarnings() {
			...
			warnings.forEach( ( warning ) => {
				const message =
					warning.message ||
					this.strings.themeWarningInline ||
					'Theme step failed — conversion continued using current theme. Update WordPress to use this theme.';
		...
		renderThemeCard( theme ) {
			const isSelected =
				( this.state.selectedThemeSlug ||
					this.getCurrentThemeSlug() ) === theme.slug;
			const isActive =
		...
		renderThemeStep() {
			...
			const currentTheme = {
				...
				screenshot:
					this.themes.currentTheme &&
					this.themes.currentTheme.screenshot
						? this.themes.currentTheme.screenshot
						: '',
			};
			...
			const selectedSummary = this.willChangeTheme()
				? formatString(
						this.strings.themeSelectedSummary || 'Selected: %s',
						selectedName
				  )
				: formatString(
						this.strings.themeUsingCurrentSummary ||
							'Using current theme: %s',
						this.getCurrentThemeName()
				  );
			container.appendChild(
				createElement(
		...
		renderSelectStep() {
			...
			if ( ! this.pages.length ) {
				container.appendChild(
					createElement(
						'p',
						null,
						this.strings.noPagesFound || 'No Elementor pages found.'
					)
				);
				return container;
			}
			...
			searchInput.placeholder =
				this.strings.searchPlaceholder || 'Search by title\u2026';
			...
			const filterOptions = [
				...
				{
					value: 'eligible',
					label: this.strings.filterEligible || 'Eligible',
				},
				{
					value: 'not_converted',
					label: this.strings.filterUnconverted || 'Unconverted',
				},
				{
					value: 'converted',
					label: this.strings.filterConverted || 'Converted',
				},
				{
					value: 'failed',
					label: this.strings.filterFailed || 'Failed',
				},
			];
			...
			selectEligibleLink.textContent =
				this.strings.selectAllEligible || 'Select all eligible';
			...
			if ( this.postTypes.length > 1 ) {
				...
				masterCheckbox.checked =
					allPageIds.length > 0 &&
					allPageIds.every( ( id ) =>
						this.state.selectedPageIds.has( id )
					);
				masterCheckbox.addEventListener( 'change', () => {
					if ( masterCheckbox.checked ) {
						this.pages.forEach( ( p ) => {
							this.state.selectedPageIds.add( p.id );
						} );
					} else {
						this.state.selectedPageIds = new Set();
					}
				...
				masterLabel.appendChild(
					createElement(
						'span',
						null,
						this.strings.selectAllAcrossTypes ||
							'Select all across all types'
					)
				);
			...
			const allVisibleSelected = visiblePages.every( ( page ) =>
				this.state.selectedPageIds.has( page.id )
			);
			selectAllCheckbox.checked =
				visiblePages.length > 0 && allVisibleSelected;
			...
			visiblePages.forEach( ( page ) => {
				...
				const badgeInfo =
					STATUS_BADGES[ page.conversionStatus ] ||
				...
				if ( hasAnyWarning ) {
					...
					if ( unsupTypes.length > 0 ) {
						const totalCount = Object.values( unsupported ).reduce(
							( s, n ) => s + n,
							0
						);
						const badge = createElement(
		...
		renderTemplatesGroup( type, label ) {
			...
			const defaultId =
				type === 'header'
					? this.state.defaultHeaderId
					: this.state.defaultFooterId;

			if ( ! templates.length ) {
				const noneMessage =
					type === 'header'
						? this.strings.noHeadersFound ||
						  'No header templates detected.'
			...
			templates.forEach( ( template ) => {
				...
				const statusTd = document.createElement( 'td' );
				const badgeInfo =
					STATUS_BADGES[ template.conversionStatus ] ||
					STATUS_BADGES.not_converted;
				const badgeLabel = badgeInfo
					? this.strings[ badgeInfo.labelKey ] || badgeInfo.labelKey
			...
			const labelText =
				type === 'header'
					? this.strings.defaultHeaderLabel ||
					  'Default header after conversion'
			...
			if ( selectedTemplates.length ) {
				selectedTemplates.forEach( ( template ) => {
					...
					optionLabel.appendChild(
						createElement( 'span', null, template.title )
					);
				...
				const message =
					type === 'header'
						? this.strings.noHeadersSelected ||
						  'No headers selected for conversion.'
		...
		renderTemplatesStep() {
			...
			headerSection.appendChild(
				this.renderTemplatesGroup( 'header', '' )
			);
			...
			footerSection.appendChild(
				this.renderTemplatesGroup( 'footer', '' )
			);
		...
		renderPagination() {
			const totalPages = Math.max(
				1,
				Math.ceil( this.getFilteredPages().length / this.state.perPage )
			);
			if ( totalPages <= 1 ) {
				return null;
			}
			...
			pagination.appendChild(
				createElement(
					'span',
					null,
					this.state.tablePage + ' / ' + totalPages
				)
			);
		...
		getFilteredPages() {
			...
			if ( f === 'eligible' ) {
				pages = pages.filter( function ( page ) {
					return page.conversionStatus !== 'converted';
				} );
			} else if ( f === 'converted' ) {
				pages = pages.filter( function ( page ) {
					return page.conversionStatus === 'converted';
				...
				pages = pages.filter( function ( page ) {
					return page.conversionStatus === 'error';
				} );
			} else if ( f === 'not_converted' ) {
				pages = pages.filter( function ( page ) {
					return page.conversionStatus === 'not_converted';
				} );
			}
			return pages;
		}
		...
		getVisiblePages() {
			...
			return this.getFilteredPages().slice(
				start,
		...
		getPageStatusLabel( conversionStatus ) {
			if ( conversionStatus === 'converted' ) {
				return (
					this.strings.statusAlreadyConverted || 'Already converted'
				);
			}
			...
			return badgeInfo
				? this.strings[ badgeInfo.labelKey ] || conversionStatus
				: conversionStatus;
		}
		...
		normalizeResultMessage( message ) {
			...
			if (
				message.indexOf(
					'conversion produced no Gutenberg content'
				) !== -1
			) {
				return (
					this.strings.errorNoOutput ||
					'No Gutenberg output was generated. The source may contain unsupported widgets or empty content.'
				);
			}
			return message;
		}
		...
		renderConflictStep() {
			...
			const summary = formatString(
				this.strings.conflictDetected ||
					'%1$d selected pages already have a converted version.',
				count
			);
			container.appendChild(
			...
			const options = [
				{
					key: 'overwrite',
					label:
						this.strings.conflictOverwrite ||
						'Update existing pages in place (overwrite)',
				},
				{
					key: 'duplicate',
					label:
						this.strings.conflictDuplicate ||
		...
		renderReviewStep() {
			...
			const selectedCount = this.state.selectedPageIds.size;
			const convertedSelected = this.getSelectedPages().filter(
				( page ) => page.conversionStatus === 'converted'
			).length;
			const convertCount = this.state.skipConverted
				? Math.max( 0, selectedCount - convertedSelected )
				: selectedCount;
			...
			stats.appendChild(
				this.buildReviewStatTile(
					convertCount,
					this.strings.reviewStatPages || 'Pages to convert'
				)
			);
			if ( headerCount || footerCount ) {
				stats.appendChild(
					this.buildReviewStatTile(
						headerCount,
						this.strings.reviewStatHeaders || 'Headers'
					)
				);
				stats.appendChild(
					this.buildReviewStatTile(
						footerCount,
						this.strings.reviewStatFooters || 'Footers'
					)
				);
			}
			if ( skippedCount > 0 ) {
				stats.appendChild(
					this.buildReviewStatTile(
						skippedCount,
						this.strings.reviewStatSkipped || 'To skip'
					)
				);
			}
					...
					( body ) => {
						...
						const modeLabel =
							this.state.mode === 'auto'
								? this.strings.modeAutoTitle ||
								  'Convert all pages automatically'
								: this.strings.modeCustomTitle ||
								  'Choose specific pages';
						ul.appendChild(
							createElement( 'li', null, modeLabel )
						);
						ul.appendChild(
							createElement(
								'li',
								null,
								formatString(
									'%1$d pages selected, %2$d will convert, %3$d skipped',
									selectedCount,
									convertCount,
									skippedCount
								)
						...
						( body ) => {
							...
							ul.appendChild(
								createElement(
									...
									null,
									formatString(
										this.strings.headerFooterSummary ||
											'%1$d headers and %2$d footers selected for conversion.',
										headerCount,
										footerCount
									)
								)
							);
							const defaultHeader = headerCount
								? this.getTemplateById(
										this.state.defaultHeaderId
								  )
								: null;
							const defaultFooter = footerCount
								? this.getTemplateById(
										this.state.defaultFooterId
								  )
								: null;
							const headerTitle = defaultHeader
								? defaultHeader.title
								: '—';
							const footerTitle = defaultFooter
								? defaultFooter.title
								: '—';
							ul.appendChild(
								createElement(
									...
									null,
									formatString(
										this.strings.headerFooterDefaults ||
											'Default header: %1$s — Default footer: %2$s',
										headerTitle,
										footerTitle
									)
								)
					...
					this.buildReviewSection(
						...
						( body ) => {
							...
							switch ( this.state.conflictPolicy ) {
								...
								default:
									policyLabel =
										this.strings.conflictSkip ||
										'Skip those pages';
							}
							const ul = document.createElement( 'ul' );
							ul.appendChild(
								createElement( 'li', null, policyLabel )
							);
							body.appendChild( ul );
						}
					)
			...
			safety.appendChild(
				createElement(
					'span',
					null,
					this.strings.safetyNote ||
						'Recommended to run on a staging environment if your site is live. Conversion runs in the background — you can safely close this page.'
				)
			);
		...
		renderProgressStep() {
			...
			if ( ! this.state.job ) {
				container.appendChild(
					createElement(
						'p',
						null,
						this.strings.processing || 'Processing…'
					)
				);
				return container;
			}
			...
			const skippedCount =
				job.counts && job.counts.skipped ? job.counts.skipped : 0;
			const errorCount =
				job.counts && job.counts.error ? job.counts.error : 0;
			...
			summary.appendChild(
				makeTile(
					successCount,
					this.strings.converted || 'Converted',
					successCount > 0 ? 'success' : 'muted'
				)
			);
			summary.appendChild(
				makeTile(
					skippedCount,
					this.strings.skipped || 'Skipped',
					'muted'
				)
			);
			summary.appendChild(
				makeTile(
					errorCount,
					this.strings.errors || 'Errors',
					errorCount > 0 ? 'error' : 'muted'
				)
			);
			summary.appendChild(
				makeTile(
			...
			if ( job.status === 'completed' && this.config.feedbackEnabled ) {
				...
				const fbRunBtn = this.buildActionPill( {
					...
					onClick: () => {
						const allIds = ( this.state.job.results || [] )
							.map( function ( r ) {
								return Number( r.id );
							} )
							.filter( Boolean );
			...
			if ( job.status === 'completed' ) {
				message =
					errorCount > 0
						? formatString(
								this.strings.jobCompletedWithErrors ||
									'Conversion finished with issues in %s.',
								formatDuration( job.duration )
						  )
						: formatString(
								this.strings.jobCompleted ||
									'Conversion completed successfully in %s.',
								formatDuration( job.duration )
						  );
			} else {
		...
		buildResultsTable( results ) {
			...
			results.forEach( ( result ) => {
				...
				if (
					...
					cb.type = 'checkbox';
					cb.checked = this.state.feedbackCheckedIds.has(
						Number( result.id )
					);
				...
				const metaParts = [];
				const typeLabel =
					result.type === 'page' && result.postTypeLabel
				...
				const badgeInfo =
					STATUS_BADGES[ resultConfig.badge ] ||
					STATUS_BADGES.not_converted;
				const badge = createElement(
					'span',
				...
				statusTd.appendChild( badge );
				const displayMessage = this.normalizeResultMessage(
					result.message
				);
				if ( displayMessage ) {
					statusTd.appendChild(
						createElement( 'div', null, displayMessage )
					);
				}
				tr.appendChild( statusTd );
				...
				if ( result.viewUrl ) {
					actionGroup.appendChild(
						this.buildActionPill( {
							...
							title:
								this.strings.viewConvertedTooltip ||
								'View converted page',
							iconPath: [
				...
				if ( result.status === 'error' && result.type === 'page' ) {
					actionGroup.appendChild(
						this.buildActionPill( {
							...
							title:
								this.strings.retryTooltip ||
								'Retry this conversion',
							iconPath: [
								'M1 4v6h6',
				...
				if (
					...
					actionGroup.appendChild(
						this.buildActionPill( {
							variant: 'feedback',
							label:
								this.strings.feedbackButtonItem || 'Feedback',
							title:
								this.strings.feedbackButtonItem ||
								'Send feedback for this item',
							iconPath: [
								'M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z',
							],
							onClick: () => {
								this.openItemFeedbackModal(
									Number( result.id ),
									result.title || '',
									String( this.state.job.id )
								);
							},
						} )
					);
		...
		renderResultsTable() {
			if (
				! this.state.job ||
				! Array.isArray( this.state.job.results ) ||
				! this.state.job.results.length
			) {
				return null;
			}
			const results = this.state.job.results;
			const errors = results.filter( function ( r ) {
				return r.status === 'error' || r.status === 'partial';
			} );
		...
		openFeedbackModal( sourceIds ) {
			...
			const job = this.state.job;
			if ( ! job || ! this.config.feedbackNonce ) {
				return;
			}
			...
			modal.style.cssText =
				'background:#fff;border-radius:8px;padding:28px 32px;max-width:520px;width:100%;max-height:85vh;overflow-y:auto;box-shadow:0 12px 48px rgba(0,0,0,0.22);';

			const h2 = document.createElement( 'h2' );
			h2.style.cssText =
				'margin:0 0 20px;font-size:17px;font-weight:600;color:#1d2327;';
			h2.textContent =
				this.strings.feedbackModalTitle || 'How did the conversion go?';
			modal.appendChild( h2 );

			// Issue type dropdown
			const issueWrap = createElement( 'div', null );
			issueWrap.style.cssText = 'margin-bottom:16px;';
			const issueLbl = createElement(
				'label',
				null,
				this.strings.feedbackIssueLabel || 'Issue type'
			);
			...
			[
				...
				[
					'layout',
					this.strings.feedbackIssueLayout || 'Layout issue',
				],
				[
					'missing_content',
					this.strings.feedbackIssueMissing || 'Missing content',
				],
				[
					'unsupported_widget',
					this.strings.feedbackIssueWidget || 'Unsupported widget',
				],
				[
					'css_styling',
					this.strings.feedbackIssueCss || 'CSS/styling',
				],
				[
					'ai_quality',
					this.strings.feedbackIssueAi || 'AI output quality',
				],
			...
			const detailLbl = createElement(
				'label',
				null,
				this.strings.feedbackIssueDetailLabel || 'Describe the issue'
			);
			...
			detailInput.style.cssText =
				'width:100%;padding:6px 8px;border:1px solid #c3c4c7;border-radius:4px;font-size:13px;box-sizing:border-box;';
			...
			const noteLbl = createElement(
				'label',
				null,
				this.strings.feedbackNoteLabel || 'Any additional notes?'
			);
			...
			submitBtn.textContent =
				this.strings.feedbackSubmit || 'Send Feedback';
			...
			submitBtn.addEventListener( 'click', function () {
				...
				submitBtn.textContent =
					self.strings.feedbackSending || 'Sending…';
				errSpan.textContent = '';
				...
				sourceIds.forEach( function ( id ) {
					fd.append( 'selected_source_ids[]', String( id ) );
				} );
					...
					.then( function ( data ) {
						if ( data.success ) {
							...
							self.showFeedbackConfirm(
								formatString(
									self.strings.feedbackSuccess ||
										'Thank you! Feedback submitted (ID: %s).',
									fbId
								)
							);
						} else {
							...
							submitBtn.textContent =
								self.strings.feedbackSubmit || 'Send Feedback';
						}
					} )
					.catch( function ( err ) {
						errSpan.textContent =
		...
		openItemFeedbackModal( sourceId, title, jobId ) {
			...
			modal.setAttribute( 'aria-modal', 'true' );
			modal.style.cssText =
				'background:#fff;border-radius:8px;padding:28px 32px;max-width:400px;width:100%;max-height:85vh;overflow-y:auto;box-shadow:0 12px 48px rgba(0,0,0,0.22);';

			const h2 = document.createElement( 'h2' );
			h2.style.cssText =
				'margin:0 0 4px;font-size:17px;font-weight:600;color:#1d2327;';
			h2.textContent =
				this.strings.feedbackItemTitle || 'How did this page convert?';
			modal.appendChild( h2 );

			if ( title ) {
				const sub = createElement( 'p', null, title );
				sub.style.cssText =
					'margin:0 0 16px;font-size:12px;color:#787c82;';
			...
			submitBtn.addEventListener( 'click', function () {
				...
				if ( noteTA.value ) {
					fd.append( 'item_notes[' + sourceId + ']', noteTA.value );
				}
				fd.append( 'user_agent', navigator.userAgent );
				fd.append( 'screen_width', String( window.screen.width ) );
				fd.append( 'screen_height', String( window.screen.height ) );
		...
		showFeedbackConfirm( message ) {
			this.state.notice = { type: 'success', message };
			this.render();
			const capturedMsg = message;
			setTimeout( () => {
				if (
					this.state.notice &&
					this.state.notice.message === capturedMsg
				) {
					this.state.notice = null;
					this.render();
				}
		...
		showAiOverlay( pageTitle, current, total, stage ) {
			...
			track.setAttribute( 'stroke', '#2271b1' );
			...
			arc.setAttribute( 'stroke', '#2271b1' );
			...
			title.style.cssText =
				'display:block;font-size:16px;font-weight:700;color:#1d2327;';
			card.appendChild( title );

			// Stage dots
			const stageStrip = document.createElement( 'div' );
			stageStrip.style.cssText =
			...
			stageKeys.forEach( function ( k, i ) {
				if ( i > 0 ) {
					...
					sep.style.cssText =
						'width:18px;height:1px;background:#dcdcde;';
					stageStrip.appendChild( sep );
				}
				const dot = document.createElement( 'span' );
				dot.style.cssText =
					'width:8px;height:8px;border-radius:50%;background:#dcdcde;transition:background 200ms ease;';
				dots[ k ] = dot;
			...
			msg.style.cssText =
				'display:block;margin-top:2px;font-size:13px;color:#2271b1;font-weight:600;line-height:1.5;';
			...
			sep.style.cssText =
				'width:100%;height:1px;background:#f0f0f1;margin-top:4px;';
			card.appendChild( sep );

			const sub = document.createElement( 'span' );
			sub.textContent = pageTitle;
			sub.style.cssText =
				'display:block;font-size:13px;color:#50575e;max-width:320px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;';
			card.appendChild( sub );

			const counter = document.createElement( 'span' );
			counter.textContent = current + ' / ' + total;
			counter.style.cssText =
				'display:inline-block;padding:3px 12px;border-radius:999px;background:#f0f6fc;border:1px solid #bcd7f0;color:#135e96;font-size:12px;font-weight:600;';
		...
		renderAiImproveStep() {
			...
			if ( ! ai || ! ai.started ) {
				...
				panel.appendChild(
					makeRow(
						apiConfigured,
						apiConfigured
							? this.strings.aiReadinessApiValid ||
									'API key configured'
							: this.strings.aiReadinessApiInvalid ||
				...
				panel.appendChild(
					makeInfoRow(
						formatString(
							this.strings.aiReadinessCredits ||
				...
				if ( ! apiConfigured ) {
					...
					apiAlert.appendChild(
						document.createTextNode(
							this.strings.aiReadinessApiMissing ||
				...
				text.appendChild(
					createElement(
						'strong',
						null,
						this.strings.aiImproveWarningTitle ||
							'AI credits will be used'
					)
				);
				text.appendChild(
					createElement(
			...
			if ( ai.started ) {
				const done = ai.pages.filter(
					( p ) => p.status === 'done'
				).length;
				const failed = ai.pages.filter(
					( p ) => p.status === 'failed'
				).length;
				const skipped = ai.pages.filter(
					( p ) => p.status === 'skipped'
				).length;
				...
				const chipData = [
					{
						count: done,
						label: this.strings.aiStatusDone || 'Done',
						cls: 'chip-done',
					},
					{
						count: failed,
						label: this.strings.aiStatusFailed || 'Failed',
						cls: 'chip-failed',
					},
					{
						count: skipped,
						label: this.strings.aiStatusSkipped || 'Skipped',
						cls: 'chip-skipped',
					},
					{
						count: total - settled - failed,
						label: this.strings.aiStatusPending || 'Pending',
						cls: 'chip-pending',
					},
				];
				chipData.forEach( ( { count, label: chipLabel, cls } ) => {
					...
					chip.appendChild(
						document.createTextNode( ' ' + chipLabel )
					);
			...
			if ( ai.finished ) {
				...
				summary.appendChild(
					createElement(
						...
						null,
						allGood
							? this.strings.aiImproveFinishedOk ||
									'All items improved successfully.'
							: this.strings.aiImproveFinishedErr ||
		...
		buildWarningPopover( page ) {
			...
			if ( unsupTypes.length > 0 ) {
				...
				sTitle.textContent =
					'⚠ ' +
					( this.strings.warnTitleUnsupported ||
						'Unsupported Widgets' );
			...
			if ( warnings.hasDynamic ) {
				...
				sTitle.textContent =
					'◎ ' +
					( this.strings.warnTitleDynamic || 'Dynamic Content' );
				sec.appendChild( sTitle );
				sec.appendChild(
					createElement(
						'p',
