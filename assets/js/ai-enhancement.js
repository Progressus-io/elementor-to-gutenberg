(function () {
  ...
  EtgAiEnhancement.prototype.renderStatsBar = function () {
    ...
    var enhanced = pages.filter(function (p) {
  ...
  EtgAiEnhancement.prototype.renderSelectionStep = function () {
    ...
    if (!this.config.aiConfigured) {
      ...
      var noticeText = document.createTextNode(
        (this.strings.noApiMessage || "A Claude API key is required.") + " ",
      );
      var noticeLink = cel(
        "a",
        "",
    ...
    this.pages.forEach(function (page) {
      ...
      if (page.sourceId) {
        ...
        srcLink.href =
          self.config.editBaseUrl + String(page.sourceId) + "&action=edit";
  ...
  EtgAiEnhancement.prototype.renderAiImproveStep = function () {
    ...
    if (!ai.started) {
      ...
      var apiLbl = cel(
        "span",
        "ele2gb-ai-readiness-status" + (cfg.aiConfigured ? "" : " is-invalid"),
        cfg.aiConfigured
      ...
      ai.pages.forEach(function (page) {
        ...
        if (fullData && fullData.lastImproved) {
          titleRow.appendChild(
            cel("span", "ele2gb-preflight-improved-badge", "✓ Enhanced"),
          );
        }
    ...
    // Progress view
    var done = ai.pages.filter(function (p) {
      return p.status === "done";
    }).length;
    var failed = ai.pages.filter(function (p) {
      return p.status === "failed";
    }).length;
    var skipped = ai.pages.filter(function (p) {
      return p.status === "skipped";
    }).length;
    var pending = ai.pages.filter(function (p) {
      return p.status === "pending";
    }).length;
    var total = ai.pages.length;
    var pct =
      total > 0 ? Math.round(((done + failed + skipped) / total) * 100) : 0;
    ...
    if (skipped > 0) {
      chips.appendChild(
        cel(
          ...
          fmt(this.strings.aiStatusSkipped || "Skipped (%1$d)", skipped),
        ),
      );
    }
    if (pending > 0) {
      chips.appendChild(
        cel(
          ...
          fmt(this.strings.aiStatusPending || "Pending (%1$d)", pending),
        ),
      );
    }
  ...
  EtgAiEnhancement.prototype.openAiFeedbackModal = function (
    ...
    submitBtn.addEventListener("click", function () {
      ...
      fetch(self.config.ajaxUrl, {
          ...
          if (data.success) {
            ...
            var msg =
              data.data && data.data.error
                ...
                : str.feedbackError || "An unexpected error occurred.";
        ...
        .catch(function (err) {
          errSpan.textContent =
            err.message || str.feedbackError || "An unexpected error occurred.";
          submitBtn.disabled = false;
          cancelBtn.disabled = false;
          submitBtn.textContent = str.feedbackSubmit || "Send Feedback";
        });
